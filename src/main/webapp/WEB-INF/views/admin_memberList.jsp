...
<body>	<%
		...
		<div class="main-content container col-md-9">
			...
			<div id="noticeSearch" class="noticeSearch">
				<select id="searchType" name="searchType">
					<option value="id"
						<c:if test="${page.searchType eq 'id'}">selected</c:if>>ID</option>
					<option value="name"
						<c:if test="${page.searchType eq 'name'}">selected</c:if>>이름</option>
					<option value="idName"
						<c:if test="${page.searchType eq 'idName'}">selected</c:if>>ID+이름</option>
  		...
  		<span>
  			<a href="/board/listPage?num=${num}">${num}</a>
				...
				<div style="display: block; text-align: center;">
					...
					<c:if test="${page.prev}">
						<span>[ <a
							href="/admin_memberList?num=${page.startPageNum - 1}${page.searchTypeKeyword}">이전</a>
							]
						</span>
					</c:if>
					...
					<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}"
						...
						<span> <c:if test="${select != num}">
								<a href="/admin_memberList?num=${num}${page.searchTypeKeyword}">${num}</a>
					...
					<c:if test="${page.next}">
						<span>[ <a
							href="/admin_memberList?num=${page.endPageNum + 1}${page.searchTypeKeyword}">다음</a>
							]
						</span>
					</c:if>
	...
	<script>
		document.getElementById("searchButton").onclick = function() {
			...
			location.href = "/admin_memberList?num=1" + "&searchType="
...
<script>
	document.getElementById("searchBtn").onclick = function() {
		...
		location.href = "/admin_memberList?num=1" + "&searchType=" + searchType
				+ "&keyword=" + keyword;
	};
</script>
 <%
