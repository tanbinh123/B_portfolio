				...
				<table class="table">
					...
					<tbody>
						<c:forEach var="dto" items='${ qna_list }'>
							<tr>
								<td>${dto.qna_idx}</td>
								<td><a href="admin_qna_view?qna_idx=${ dto.qna_idx }">${ dto.qna_title }</a></td>
								<td>${ dto.qna_name }</td>
...
<div class="d-flex flex-row justify-content-center">
			<c:if test="${page.prev}">
				<span>[ <a
					href="/admin_qna?num=${page.startPageNum - 1}${page.searchTypeKeyword}">이전</a>
					]
				</span>
			</c:if>
			...
			<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}"
				...
				<span> <c:if test="${select != num}">
						<a href="/admin_qna?num=${num}${page.searchTypeKeyword}">${num}</a>
			...
			<c:if test="${page.next}">
				<span>[ <a
					href="/admin_qna?num=${page.endPageNum + 1}${page.searchTypeKeyword}">다음</a>
					]
				</span>
			</c:if>
</div>
...
<script>
	document.getElementById("searchButton").onclick = function () {
		...
		location.href = "/admin_qna?num=1" + "&searchType=" + searchType + "&keyword=" + keyword;
	};
</script>
