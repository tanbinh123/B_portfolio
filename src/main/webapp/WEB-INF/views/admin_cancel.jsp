...
<body>
	...
	<div id="content" class="d-flex flex-row bd-highlight mb-3">
		...
		<div class="main-content container col-md-9">
			...
			<!-- 공지사항 검색 -->
			<div id="cancelSearch" class="cancelSearch">
					<select id="searchType" name="searchType">
						...
						<option value="id"
							<c:if test="${page.searchType eq 'id'}">selected</c:if>>아이디</option>
					</select>
...
<div>
	...
	<%-- <c:forEach begin="1" end="${pageNum}" var="num">
  		<span>
			...
			<table class="table">
				...
				<tbody>
					<c:forEach var="dto" items='${ list }'>
						<tr>
							<td>${dto.cancel_idx}</td>
							<td><a
								href="admin_cancel_view?cancel_idx=${ dto.cancel_idx }">${dto.cancel_title}</a></td>
			...
			<div style="display: block; text-align: center;">
				...
				<c:if test="${page.prev}">
		<span>[ <a href="/admin_cancel?num=${page.startPageNum - 1}${page.searchTypeKeyword}">이전</a> ]</span>
	</c:if>
	...
	<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}" var="num">
		<span>
			...
			<c:if test="${select != num}">
				<a href="/admin_cancel?num=${num}${page.searchTypeKeyword}">${num}</a>
			</c:if> 			
	...
	<c:if test="${page.next}">
		<span>[ <a href="/admin_cancel?num=${page.endPageNum + 1}${page.searchTypeKeyword}">다음</a> ]</span>
	</c:if>
...
<script>
	...
	document.getElementById("searchButton").onclick = function () {
		...
		location.href = "/admin_cancel?num=1" + "&searchType=" + searchType + "&keyword=" + keyword;
	};
</script>
</body>
